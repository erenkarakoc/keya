...
const getPropertyFromSahibinden = (pasted: string, setFieldValue: any, setCurrentPrice: any, setCurrentDues: any, setDescription: any, setMarkerPosition: any, map: any) => {
  ...
  const addressContent = content.querySelector(".getDirectionsButton")?.querySelector("a")?.getAttribute("href");
  if (addressContent) {
  const [addressLatContent, addressLngContent] = addressContent.split('/').pop()?.split(',') ?? []
  ...
  const roomContent = Array.from(infoList).find((li) => li.textContent?.includes("Oda Sayısı"))?.querySelector("span")?.innerHTML.trim()
  setFieldValue("propertyDetails.room", roomContent ?? "")

  const deedStatusContent = Array.from(infoList).find((li) => li.textContent?.includes("Tapu Durumu"))?.querySelector("span")?.innerHTML.trim()
  const deedStatus = deedStatusContent?.includes("Kat Mülkiyetli") ? "condominium" : deedStatusContent?.includes("Kat İrtifaklı") ? "floorAltitude" : deedStatusContent?.includes("Hisseli Tapulu") ? "shareTitleDeed" : deedStatusContent?.includes("Müstakil Tapulu") ? "detachedTitleDeed" : deedStatusContent?.includes("Arsa Tapulu") ? "landTitleDeed" : ""
  setFieldValue("propertyDetails.deedStatus", deedStatus)

  const squareGrossContent = Array.from(infoList).find((li) => li.textContent?.includes("(Brüt)"))?.querySelector("span")?.innerHTML.trim()
  setFieldValue("propertyDetails.squareGross", squareGrossContent ?? "")

  const squareNetContent = Array.from(infoList).find((li) => li.textContent?.includes("(Net)"))?.querySelector("span")?.innerHTML.trim()
  setFieldValue("propertyDetails.squareNet", squareNetContent ?? "")
  
  const parkingLotContent = Array.from(infoList).find((li) => li.textContent?.includes("Otopark"))?.querySelector("span")?.innerHTML.trim()
  const parkingLot = parkingLotContent?.includes("Açık & Kapalı Otopark") ? "openNclosedParkingLot" : parkingLotContent?.includes("Açık Otopark") ? "openParkingLot" : parkingLotContent?.includes("Kapalı Otopark") ? "closedParkingLot" : ""
  setFieldValue("propertyDetails.parkingLot", parkingLot ?? "")
  
  const heatingContent = Array.from(infoList).find((li) => li.textContent?.includes("Isıtma"))?.querySelector("span")?.innerHTML.trim()
  const heating = heatingContent?.includes("Soba") ? "stove" : heatingContent?.includes("Doğalgaz Sobası") ? "naturalGasStove" : heatingContent?.includes("Kat Kaloriferi") ? "floorRadiator" : heatingContent?.includes("Merkezi (Pay Ölçer)") ? "centerShareMeter" : heatingContent?.includes("Merkezi") ? "central" : heatingContent?.includes("Kombi (Doğalgaz)") ? "combiBoilerNaturalGas" : heatingContent?.includes("Kombi (Elektrik)") ? "combiBoilerElectricity" : heatingContent?.includes("Yerden Isıtma") ? "floorHeating" : heatingContent?.includes("Klima") ? "airConditioning" : heatingContent?.includes("Fancoil Ünitesi") ? "fancoilUnit" : heatingContent?.includes("Güneş Enerjisi") ? "solarEnergy" : heatingContent?.includes("Elektrikli Radyatör") ? "electricRadiator" : heatingContent?.includes("Jeotermal") ? "geothermal" : heatingContent?.includes("Şömine") ? "fireplace" : heatingContent?.includes("VRV") ? "VRV" : heatingContent?.includes("Isı Pompası") ? "heatPump" : "";
  setFieldValue("propertyDetails.heating", heating ?? "")
  
  const duesContent = Array.from(infoList).find((li) => li.textContent?.includes("Aidat"))?.querySelector("span")?.innerHTML.trim()
  setFieldValue("propertyDetails.dues", duesContent != "Belirtilmemiş" ? duesContent : "")
  setCurrentDues(duesContent != "Belirtilmemiş" ? duesContent ?? "" : "")
  
  const bathroomContent = Array.from(infoList).find((li) => li.textContent?.includes("Banyo Sayısı"))?.querySelector("span")?.innerHTML.trim()
  setFieldValue("propertyDetails.bathroom", bathroomContent ?? "")
  
  const buildingAgeContent = Array.from(infoList).find((li) => li.textContent?.includes("Bina Yaşı"))?.querySelector("span")?.innerHTML.trim()
  setFieldValue("propertyDetails.buildingAge", buildingAgeContent ?? "")
  
  const buildingFloorsContent = Array.from(infoList).find((li) => li.textContent?.includes("Kat Sayısı"))?.querySelector("span")?.innerHTML.trim()
  setFieldValue("propertyDetails.buildingFloors", buildingFloorsContent ?? "")
  
  const buildingAtFloorContent = Array.from(infoList).find((li) => li.textContent?.includes("Bulunduğu Kat"))?.querySelector("span")?.innerHTML.trim()
  setFieldValue("propertyDetails.buildingAtFloor", buildingAtFloorContent ?? "")
  
  const balconyContent = Array.from(infoList).find((li) => li.textContent?.includes("Balkon"))?.querySelector("span")?.innerHTML.trim()
  setFieldValue("propertyDetails.balcony", balconyContent === "Var" ? "true" : "false")
  
  const elevatorContent = Array.from(infoList).find((li) => li.textContent?.includes("Asansör"))?.querySelector("span")?.innerHTML.trim()
  setFieldValue("propertyDetails.elevator", elevatorContent === "Var" ? "true" : "false")
  
  const withAccesoriesContent = Array.from(infoList).find((li) => li.textContent?.includes("Eşyalı"))?.querySelector("span")?.innerHTML.trim()
  setFieldValue("propertyDetails.withAccesories", withAccesoriesContent === "Evet" ? "true" : "false")
  
  const inComplexContent = Array.from(infoList).find((li) => li.textContent?.includes("Site İçerisinde"))?.querySelector("span")?.innerHTML.trim()
  setFieldValue("propertyDetails.inComplex", inComplexContent === "Evet" ? "true" : "false")
  
  const sahibindenNoContent = Array.from(infoList).find((li) => li.textContent?.includes("İlan No"))?.querySelector("span")?.innerHTML.trim()
