...
const PropertyEditModalForm: FC<Props> = ({ property, isPropertyLoading }) => {
    ...
    if (e.target.files && e.target.files.length > 0) {
      ...
      files.forEach((file) => {
        ...
        const storageRef = ref(
          storage,
          `images/properties/${slugify(property.name)}-${randomName}`
        )
  ...
  useEffect(() => {
    const fetchBrokers = async () => {
      try {
        ...
        if (response) {
          ...
          const owners: string[] = property.owners
    ...
    fetchBrokers()
    setUploadedImageUrls(property.photoURLs)
  }, [property])
    ...
    <>
      ...
      >
        {({ values, setFieldValue, dirty, isValid }) => (
          <Form noValidate id="property_edit_modal_form" placeholder={undefined}>
            {/* begin::Scroll */}
            <div
            ...
            <div className="text-center pt-15">
              ...
              <button
                ...
                disabled={
                  !dirty || !isValid || isPropertyLoading || submittingForm
                }
              >
